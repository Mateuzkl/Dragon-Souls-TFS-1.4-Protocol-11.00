...
doPlayerSendTextMessage(cid,MESSAGE_STATUS_WARNING,'CD: Exevo Tera')
end
end

local exhausted_seconds = 14 -- Segundos que o Player Poderá castar a spell novamente
local exhausted_storagevalue = 63425 -- Storage Value do Cool Down
...
doPlayerSendTextMessage(target,20,'Voce está sendo devorado.')
doTargetCombatCondition(0, target, condition, CONST_ME_NONE)
...
doCombat(cid, combatDist, numberToVariant(target))
	...
	if rand == 1 and isPlayer(cid) == 1 then
 	doPlayerSay(cid,"Exevo Tera!",16)
      addEvent(Cooldown, 1*14000,cid)
  ...
  addEvent(frozzen6, 3.5*1000, target)

...
if isPlayer(cid) == TRUE then
doPlayerAddHealth(cid,Kaz)
doPlayerAddMana(cid,Kaz)
doSendMagicEffect(getCreaturePosition(cid), 51)
end
end
...
if isPlayer(cid) == TRUE then
doPlayerAddHealth(cid,Haz)
doPlayerAddMana(cid,Haz)
doSendMagicEffect(getCreaturePosition(cid), 51)
end
end
...
Kaz = ((10*getPlayerMaxHealth(cid)/100)+(skill*3)+(magic*2)+(level*3))
Kaz = ((10*getPlayerMaxMana(cid)/100)+(skill*2)+(magic*3)+(level*3))
Haz = ((10*getPlayerMaxHealth(cid)/100)+(skill*2)+(magic*2)+(level*1))
Haz = ((10*getPlayerMaxMana(cid)/100)+(skill*2)+(magic*2)+(level*1))
HP = ((10*getPlayerMaxHealth(cid)/100)+(skill*3)+(magic*5)+(level*5))
MP = ((10*getPlayerMaxMana(cid)/100)+(skill*5)+(magic*3)+(level*5))

-- Config Local Exhaust --
local exhausted_seconds = 30 -- Segundos para o uso de outro Blood.
local exhausted_storagevalue = 4850 -- Número Salvo o Exhaust por Storage

-- Final da Config de Exhaust. --
...
elseif Blood == 2 and (os.time() >= getPlayerStorageValue(cid, exhausted_storagevalue)) then
...
addEvent(Blod1, 1*2000,cid)
setPlayerStorageValue(cid, exhausted_storagevalue, os.time() + exhausted_seconds)
doPlayerAddSoul(cid,-50)
doRemoveItem(item.uid,1)

elseif Blood == 3 and (os.time() >= getPlayerStorageValue(cid, exhausted_storagevalue)) then
doPlayerAddHealth(cid,10*getPlayerMaxHealth(cid)/100)
doPlayerAddMana(cid,10*getPlayerMaxHealth(cid)/100)

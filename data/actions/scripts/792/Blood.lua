...
doPlayerSendTextMessage(cid,16,'Chronos Is Gone. (Revitalization Disativaded)')
doPlayerAddHealth(cid,HP)
doPlayerAddMana(cid,MP)
doPlayerSay(cid, "Weak!", TALKTYPE_ORANGE_1)
...
if isPlayer(cid) == TRUE then
doPlayerSendTextMessage(cid,16,'Kazard Is Gone. (Full Haste Disativaded)')
doPlayerAddHealth(cid,HP)
doPlayerAddMana(cid,MP)
doSendMagicEffect(getCreaturePosition(cid), 51)
end
...
doPlayerAddHealth(cid,HP)
doPlayerAddMana(cid,MP)
doSendAnimatedText(getCreaturePosition(cid), "Silence!", TEXTCOLOR_WHITE_EXP)
doSendMagicEffect(getCreaturePosition(cid), 51)
...
if isPlayer(cid) == TRUE then
doPlayerSendTextMessage(cid,20,'Hazus Is Gone. (Reflect Disativaded)')
doPlayerAddHealth(cid,HP)
doPlayerAddMana(cid,MP)
doSendMagicEffect(getCreaturePosition(cid), 51)
end
...
doPlayerAddHealth(cid,HP)
doPlayerAddMana(cid,MP)
doPlayerSay(cid, "Haha!", TALKTYPE_ORANGE_1)
doSendMagicEffect(getCreaturePosition(cid), 51)
addEvent(Blod15, 1 * 2000, cid)
end
end
...
HP = ((10*getPlayerMaxHealth(cid)/100)+(skill*5)+(magic*5))
MP = ((10*getPlayerMaxMana(cid)/100)+(skill*2)+(magic*3))

if getPlayerSoul(cid) <= 49 then
doPlayerSendTextMessage(cid,20,'Desculpe, você não tem Souls suficiente.')
elseif (exhaust(cid, 30000, 30) > 0) then
...
doPlayerSay(cid, "Grr! Chronos Is Back!", TALKTYPE_ORANGE_1)
...
doPlayerSendTextMessage(cid,20,"Você não pode ativar um blood of god enquanto estiver em efeito de outro.")
end
end

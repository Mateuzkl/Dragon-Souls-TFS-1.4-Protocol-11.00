...
msg = string.lower(msg)
  ...
  if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
   selfSay('Hello ' .. cname .. '! Are you ready to live with us on EDRON?')
   talk_state = 0
   focus = cid
   selfLook(cid)
   talk_start = os.clock()
...
elseif msgcontains(msg, 'hi') and (focus ~= cid) and getDistanceToCreature(cid) < 4 then
 ...
 talk_start = os.clock()    if talk_state == 0 then
  if msgcontains(msg, 'yes') then  -- wanna change temple to edron
  ...
  elseif talk_state == 3 then  -- telling city name
   if msgcontains(msg, 'yes') then
    selfSay('congratulations, You Have Been Blessed and Now You Are a Citizen of Edron.')
    setPlayerMasterPos(cid,778,847,8)
    selfSay('/send ' .. cname .. ', 778 847 8')

    talk_state = 0
    focus = 0
    talk_start = 0
   else
    selfSay('Sorry, thats inst the answer. Do you want to live on EDRON with us ?')
    talk_state = 3
   end
   end
  end

...
function StdModule.TempleChange(cid, message, keywords, parameters, node)
		...
		if(isPremium(cid) or parameters.premium == false) then
			...
			elseif(doPlayerRemoveMoney(cid, parameters.cost) ~= TRUE) then
				npcHandler:say('You do not have enough money!')
          				...
          				doPlayerSetTown(cid,parameters.townid)
				doTeleportThing(cid, parameters.pos)
				doSendMagicEffect(parameters.pos, 10)
			end
		else

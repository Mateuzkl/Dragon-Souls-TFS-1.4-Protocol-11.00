...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 2 then
 		selfSay('Congratulations ' .. creatureGetName(cid) .. '! You pass on the test!')
		selfSay('Go to Calona on Carlin, and say you are a "novice", her will help you, so... You are ready to become a Archer?')
 		focus = cid
 		talk_start = os.clock()
	...
	if msgcontains(msg, 'yes') and getPlayerVocation(cid) == 0 then
		selfSay('Good luck my little Archer!')
		doPlayerSendTextMessage(cid,19,"You are now a Archer!")
	...
	elseif msgcontains(msg, 'no') then
		selfSay('Ok my love... I will wait.')
  		focus = 0
		talk_state = 0

...
local function farewellCallback(cid)
    local player = Player(cid)
    if player:hasBlessing(1) then -- Arquinothep blessing
...
local function creatureSayCallback(cid, msgType, msg)
    ...
    elseif msgcontains(msg, 'bless') then
        npcHandler:say('Hmm... So you want be blessed by UnHolly.... Are you shure about That?', cid)
    ...
    elseif npcHandler.topic[cid] == topicList.BLESS_CONFIRM and (msgcontains(msg, 'yes') or msgcontains(msg, 'sim')) then
        if player:hasBlessing(4) then
            if msgcontains(msg, 'yes') then
                ...
                npcHandler:say('Voce ja esta abencoado.', cid)
            end
        else
            if player:removeMoney(cost) then
                player:addBlessing(4)
                if msgcontains(msg, 'yes') then
                    npcHandler:say('You are Blessed by UnHolly now!', cid)
                else
                    npcHandler:say('Voce esta abencoado por UnHolly!', cid)
                end
            else
                if msgcontains(msg, 'yes') then
    ...
    elseif npcHandler.topic[cid] == topicList.BLESS_CONFIRM and (msgcontains(msg, 'no') or msgcontains(msg, 'nao')) then
        if msgcontains(msg, 'no') then
            ...
            npcHandler:say('Gostaria de algo mais?', cid)
        end
        npcHandler.topic[cid] = topicList.NONE
    ...
    elseif msgcontains(msg, 'tchau') then
        if player:hasBlessing(1) then
            npcHandler:say('Va com Deus, ' .. player:getName() .. '!', cid)
        else
            npcHandler:say('Tome cuidado ' .. player:getName() .. '...', cid)
        end

focus = 0
 ...
 function onCreatureSay(cid, type, msg)
   	...
   	if ((string.find(msg, '(%a*)oi(%a*)')) and (focus == 0)) and getDistanceToCreature(cid) < 4 then
  		if getPlayerVocation(cid) == 1 then
  			selfSay('Ola ' .. creatureGetName(cid) .. '! Que magia posso te conceder?')
  			focus = cid
  			talk_start = os.clock()
  		else
  			selfSay('So a conhecimento para magos aqui.')
  		end
  	...
  	if string.find(msg, '(%a*)oi(%a*)') and (focus ~= cid) and getDistanceToCreature(cid) < 4 then
		...
		if msgcontains(msg, 'force strike') and focus == cid then
  		...
  		if msgcontains(msg, 'ultimate explosion') and focus == cid then
		...
		if msgcontains(msg, 'invisible') and focus == cid then
 		...
 		if msgcontains(msg, 'cancel invisibility') and focus == cid then

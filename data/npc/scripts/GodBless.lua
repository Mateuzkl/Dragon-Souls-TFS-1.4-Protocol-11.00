...
local function greetCallback(cid)
    ...
    if player:getVocation():getId() >= 5 then
        if msgcontains(msg, 'hi') then
            ...
            npcHandler:say('So posso ajudar vocacoes regulares.', cid)
        end
        return false
    end
    return true
...
local function farewellCallback(cid)
    local player = Player(cid)
    if player:hasBlessing(2) then -- Arquinothep blessing
...
local function creatureSayCallback(cid, msgType, msg)
    ...
    elseif msgcontains(msg, 'bless') then
        npcHandler:say('Hmm... So you want be blessed by Gods.... Are you shure about That?', cid)
    ...
    elseif npcHandler.topic[cid] == topicList.BLESS_CONFIRM and (msgcontains(msg, 'yes') or msgcontains(msg, 'sim')) then
        if player:hasBlessing(2) then
            ...
            if player:removeMoney(cost) then
                ...
                if msgcontains(msg, 'yes') then
                    npcHandler:say('All the Gods accepted you to be blessed... So be it!', cid)
                else
                    npcHandler:say('Todos os Deuses aceitaram-te , entao que seja!', cid)
                end
            else
                if msgcontains(msg, 'yes') then

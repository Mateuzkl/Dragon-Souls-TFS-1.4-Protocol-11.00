  ...
  function onCreatureSay(cid, type, msg)
  	...
  	if ((string.find(msg, '(%a*)oi(%a*)')) and (focus == 0)) and getDistanceToCreature(cid) < 4 then
 		...
 		selfLook(cid)
  		...
  		if getPlayerVocation(cid) >= 5 then
  		selfSay('So posso ajudar vocacoes regulares.')
  	end
	end
	...
	if string.find(msg, '(%a*)oi(%a*)') and (focus ~= cid) and getDistanceToCreature(cid) < 4 then
  	...
  	if ((string.find(msg, '(%a*)sim(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusb(cid) == 0 then
 		bless = creatureGetBless(cid) + 1
 		blessa = creatureGetBlessStatusa(cid)
 		blessc = creatureGetBlessStatusc(cid)
 		blessd = creatureGetBlessStatusd(cid)
		 ...
		 if pay(cid,cost) then
    		setNewBless(focus, cost, blesse, blessd, blessc, blessb, blessa, bless)
  		selfSay('Todos os Deuses aceitaram-te , entao que seja!')
 	else
  	...
  	if ((string.find(msg, '(%a*)sim(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusb(cid) >= 1 then
  	...
  	if ((string.find(msg, '(%a*)nao(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusb(cid) >= 0 then
  		selfSay('Algo mais?')
 		talk_start = 0
 	end
  	...
  	if string.find(msg, '(%a*)tchau(%a*)') and focus == cid and getDistanceToCreature(cid) < 4 and creatureGetBlessStatusb(cid) >= 1 then
  	...
  	if string.find(msg, '(%a*)tchau(%a*)') and focus == cid and getDistanceToCreature(cid) < 4 and creatureGetBlessStatusb(cid) == 0 then

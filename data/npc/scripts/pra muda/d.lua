  ...
  function onCreatureSay(cid, type, msg)
	...
	if string.find(msg, '(%a*)oi(%a*)') and (focus ~= cid) and getDistanceToCreature(cid) < 4 then
  	...
  	if ((string.find(msg, '(%a*)bless(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 then
  		selfSay('Hmm... So you want be blessed by UnHolly.... Are you shure about That?')
 		talk_start = 1
 	end
  	...
  	if ((string.find(msg, '(%a*)yes(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusd(cid) == 0 then
 		...
 		if pay(cid,cost) then
 			selfSay('/bless4 ' .. creatureGetName(cid) .. '')
 			selfSay('You are Blessed by UnHolly now!')
 		else
  	...
  	if ((string.find(msg, '(%a*)sim(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusd(cid) == 0 then
 		bless = creatureGetBless(cid) + 1
 		blessb = creatureGetBlessStatusb(cid)
 		blessc = creatureGetBlessStatusc(cid)
 		blessd = creatureGetBlessStatusd(cid)
 		...
 		if pay(cid,cost) then
 			selfSay('/bless4 ' .. creatureGetName(cid) .. '')
 			selfSay('Voce esta abencoado por UnHolly!')
 		else

...
local destinations = {
    ['desert city'] = {cost = 50, pos = Position(369, 270, 7)},
    ['devcity'] = {cost = 20, pos = Position(171, 60, 6)}
}

local function greetCallback(cid)
    local player = Player(cid)
    if player:isPremium() then
        npcHandler:say('Hello ' .. player:getName() .. '! I can take you to the Desert City (50gp) or DevCity (20gp). Where do you want to go?', cid)
        return true
    else
...
npcHandler:setMessage(MESSAGE_GREET, 'Hello |PLAYERNAME|! I can take you to the Desert City (50gp) or DevCity (20gp). Where do you want to go?')

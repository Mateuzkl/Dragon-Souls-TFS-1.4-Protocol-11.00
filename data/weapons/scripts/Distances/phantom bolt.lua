...
combat2:setParameter(COMBAT_PARAM_DISTANCEEFFECT, CONST_ANI_ONYXARROW)

function onGetFormulaValues2(player, level, maglevel)
    local skill = player:getSkillLevel(SKILL_DISTANCE)
    local min = -((skill*12)+level)
    local max = -((skill*14)+level)
    return min, max
end
...
combat3:setParameter(COMBAT_PARAM_DISTANCEEFFECT, CONST_ANI_ONYXARROW)

function onGetFormulaValues3(player, level, maglevel)
    local skill = player:getSkillLevel(SKILL_DISTANCE)
    local min = -((skill*12)+level)
    local max = -((skill*16)+level)
    return min, max
end
...
condition2:setParameter(CONDITION_PARAM_SPEED, -400)
...
condition3:setParameter(CONDITION_PARAM_SPEED, -400)
...
function onUseWeapon(player, variant)
    ...
    if rand == 1 then
        ...
        player:addHealth(math.random(40, 400))

...
function onGetFormulaValues2(player, level, maglevel)
    local skill = player:getSkillLevel(SKILL_DISTANCE)
    local min = -((skill*9)+level)
    local max = -((skill*12)+level)
    return min, max
end
...
combat3:setParameter(COMBAT_PARAM_DISTANCEEFFECT, CONST_ANI_INFERNALBOLT)
...
function onUseWeapon(player, variant)
    ...
    if rand == 1 then
        ...
        player:addHealth(math.random(30, 300))

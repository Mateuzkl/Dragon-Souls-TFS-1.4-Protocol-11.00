...
bool IOMapSerialize::saveHouseItems()
{
	...
	console::print(CONSOLEMESSAGE_TYPE_INFO, fmt::format("Saved house items in: {:.2f} s", (OTSYS_TIME() - start) / 1000.0));
	return success;
}
...
bool IOMapSerialize::loadContainer(PropStream& propStream, Container* container)
{
	while (container->serializationCount > 0) {
		if (!loadItem(propStream, container)) {
			console::reportWarning("IOMapSerialize::loadContainer", fmt::format("Unserialization error for container item: {:d}", container->getID()));
			return false;
		}
...
bool IOMapSerialize::loadItem(PropStream& propStream, Cylinder* parent)
{
	...
	if (iType.moveable || iType.forceSerialize || !tile || iType.isCarpet()) {
		//create a new item
		...
		if (item) {
			if (item->unserializeAttr(propStream)) {
				...
				console::reportWarning("IOMapSerialize::loadItem", fmt::format("Unserialization error for item: {:d}", id));
				delete item;
				return false;
			}
		}
...
void IOMapSerialize::saveTile(PropWriteStream& stream, const Tile* tile)
{
	...
	for (Item* item : *tileItems) {
		...
		if (!(it.moveable || it.forceSerialize || it.isCarpet() || item->getDoor() || (item->getContainer() && !item->getContainer()->empty()) || it.canWriteText || item->getBed())) {
			continue;
		}

...
{
	Reflect reflect;
	reflect.percent = getField<uint16_t>(L, arg, "percent");
	reflect.chance = getField<uint16_t>(L, arg, "chance");
	return reflect;
}
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(SLOTP_ANKH)
	registerEnum(SLOTP_WING)
	registerEnum(SLOTP_AURA)
	...
	registerEnum(RETURNVALUE_SAMEIP_CANNOTATTACKPLAYER)
	...
	registerEnumIn("configKeys", ConfigManager::ITEM_STACKING_LIMIT)
...
int LuaScriptInterface::luaDoPlayerAddItem(lua_State* L)
{
	...
	while (itemCount > 0) {
		...
		uint16_t maxStack = it.getItemMaxCount();
		if (it.stackable && stackCount > maxStack) {
			stackCount = maxStack;
		}
...
int LuaScriptInterface::luaGameGetSkillStage(lua_State* L)
{
	...
	lua_pushnumber(L, g_config.getSkillStage(skilllevel));
	return 1;
}
...
int LuaScriptInterface::luaGameGetMagicLevelStage(lua_State* L)
{
	...
	lua_pushnumber(L, g_config.getMagicLevelStage(ml));
	return 1;
}
...
int LuaScriptInterface::luaGameCreateItem(lua_State* L)
{
	...
	if (it.hasSubType()) {
		if (it.stackable) {
			itemCount = std::ceil(count / (float)it.getItemMaxCount());
		}
...
int LuaScriptInterface::luaItemSetIncreasePercent(lua_State* L)
{
	// item:setIncreasePercent(combatType, percent)
	Item* item = getUserdata<Item>(L, 1);
	if (!item) {
		lua_pushnil(L);
		return 1;
	...
	item->setIncreasePercent(getNumber<CombatType_t>(L, 2), getNumber<uint16_t>(L, 3));
	pushBoolean(L, true);
	return 1;
}

int LuaScriptInterface::luaItemGetIncreasePercent(lua_State* L)
{
	// item:getIncreasePercent(combatType[, total = true])
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		lua_pushnumber(L, item->getIncreasePercent(getNumber<CombatType_t>(L, 2), getBoolean(L, 3, true)));
	} else {
		lua_pushnil(L);
	}
	return 1;

...
Player::Player(ProtocolGame_ptr p) :
	Creature(), preyData(PREY_SLOTCOUNT), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_INBOX)), client(std::move(p)), varAbsorbPercentAll(0), varLuck(0)
{
...
bool Player::updateSaleShopList(const Item* item)
{
	...
	if (itemId != ITEM_GOLD_COIN && itemId != ITEM_PLATINUM_COIN && itemId != ITEM_CRYSTAL_COIN && itemId != ITEM_RUBY_COIN) {
